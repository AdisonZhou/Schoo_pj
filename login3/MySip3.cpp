...
void MySip3::on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
	...
	//AfxMessageBox(pThis1->connect_state);
	if (pThis1->connect_state == "CONFIRMED")
	{
		//连接成功
		//AfxMessageBox(_T("ssssss"));
		pThis1->confirmed_disconnecting = CONFIRMED;
	}
	else if (pThis1->connect_state == "DISCONNCTD")
	{
		//断开连接
		//AfxMessageBox(_T("ssssss1"));
		pThis1->confirmed_disconnecting = DISCONNECTING;
...
void MySip3::make_call(pj_str_t uri,bool video_w)
{
	pjsua_call_setting call_setting;
	pjsua_call_setting_default(&call_setting);
	if (video_w == TRUE)
	{
		//AfxMessageBox(_T("makecall"));
...
void MySip3::ans_call(bool video_w)
{
	//call 设置
	pjsua_call_setting call_setting;
	pjsua_call_setting_default(&call_setting);
