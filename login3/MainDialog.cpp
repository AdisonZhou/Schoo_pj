...
BEGIN_MESSAGE_MAP(MainDialog, CDialogEx)
	...
	ON_MESSAGE(W_CHAT_WIN, chat_win_change)
	ON_MESSAGE(W_VIDEO_WIN, video_win_change)
...
void MainDialog::On21()
{
	...
	if (Sip_p.SIP_USER2 != temp)
	{
		...
		Sip_p.make_call(dst,VIDEO_AVAILABLE);
		make_call_flag = 1;
...
{
	    Sip_p.ans_call(VIDEO_AVAILABLE);
        SetTimer(timer_id2, 30, NULL);
		//window_reset();
...
LRESULT MainDialog::chat_win_change(WPARAM wParam, LPARAM lParam)
{
	int val = *((int*)lParam);
	if (val == 1)
	{
...
LRESULT MainDialog::video_win_change(WPARAM wParam, LPARAM lParam)
{
	int val = *((int*)lParam);
	return 0;
}
...
{
	//if (Sip_p.connect_state == _T("CONFIRMED")&&make_call_flag==1&& Sip_p.local_receive_flag == 1)
	//{
	//	    SetTimer(timer_id2, 3, NULL);
	//	    make_call_flag = 0;
...
void MainDialog::win_change()
{

}

void MainDialog::confirm_call()
{
	if (Sip_p.confirmed_disconnecting == CONFIRMED)
	{
		Sip_p.confirmed_disconnecting = 0;
		//连接成功
		//界面刷新
		int val =1;
		SendMessage(W_CHAT_WIN, 0, (LPARAM)(&val));
		...
		int vid_idx;
		pjsua_vid_win_id wid2 = -1;
		vid_idx = pjsua_call_get_vid_stream_idx(Sip_p.CALL_ID);
		if (vid_idx >= 0)
		{
			pjsua_call_info ci;
			pjsua_call_get_info(Sip_p.CALL_ID, &ci);
			wid2 = ci.media[vid_idx].stream.vid.win_in;
		}
		pjsua_vid_win_info inf;
		if (wid2 != -1)//显示对方视频
	...
	else if (Sip_p.confirmed_disconnecting == DISCONNECTING)
	{
		//断开连接
		Sip_p.confirmed_disconnecting = 0;
		KillTimer(timer_id2);
